                                     ...
                                     float dest_spawn_x, float dest_spawn_y)
{
    ...
    for (int r = 0; r < map->rows; r++) {
        int c = 0;
        while (c < map->cols) {
            if (map->cells[r * map->cols + c] == tile_value) {
                /* Start of a matching tile run – extend right as far as possible. */
                int run_start = c;
                while (c < map->cols &&
                       map->cells[r * map->cols + c] == tile_value)
                    c++;
...
int map_build_door_triggers(const Map *map, Location *loc,
                            ...
                            float dest_spawn_x, float dest_spawn_y)
{
    return map_build_exit_triggers_for_tile(map, loc, MAP_TILE_DOOR,
