...
int savegame_load(Game *game)
{
    ...
    int   cur_item = -1;  /* index of item being read */
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if      (strcmp(key, "VERSION")        == 0) version = atoi(val);
            ...
            cur_item++;
            if (cur_item < inv_count)
                inv_items[cur_item].id = atoi(val);
        }
    ...
    /* Update camera to the loaded location */
    if (game->world) {
    ...
    (void)version; /* reserved for future migration */
    return 1;
}
