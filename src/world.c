...
void world_setup_rooms(World *world, SDL_Renderer *renderer)
{
    ...
    for (int i = 0; i < world->location_count; i++) {
        ...
        /* ── 0: Entrance Hall ───────────────────────────────────────── */
            case 0: {
                ...
                if (m) {
                    ...
                    map_build_interactive_triggers_for_tile(m, loc, 6, 55, 0.0f, 0.0f);
        ...
        case 4: {
                ...
                if (m) {
                    map_build_colliders(m, loc);
                    /* Add solid collision for the fuel-tank pickup tile (1) and
                       the two fuel-slot tiles (2 and 6) so the player cannot
                       walk through any of these fixtures. */
