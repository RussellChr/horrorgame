...
Game *game_init(SDL_Window *window, SDL_Renderer *renderer)
{
    ...
    if (SDL_LoadWAV("assets/sfx/glass cracking.wav", &g->glass_sfx_wav_spec,
                    &g->glass_sfx_wav_buf, &g->glass_sfx_wav_len)) {
        g->glass_sfx_audio_stream = SDL_OpenAudioDeviceStream(
            SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK, &g->glass_sfx_wav_spec, NULL, NULL);
        if (!g->glass_sfx_audio_stream)
            SDL_Log("game_init: failed to open glass SFX stream: %s", SDL_GetError());
    } else {
...
void game_cleanup(Game *game)
{
    ...
    if (game->glass_sfx_audio_stream) SDL_DestroyAudioStream(game->glass_sfx_audio_stream);
    if (game->glass_sfx_wav_buf)      SDL_free(game->glass_sfx_wav_buf);
        ...
        if (p->current_location_id == LOCATION_ARCHIVE) {
            ...
            for (int i = 0; i < ARCHIVE_GLASS_COUNT; i++) {
                ...
                if (px >= gx && px <= gx + ARCHIVE_GLASS_SIZE &&
                    ...
                    if (game->glass_sfx_audio_stream && game->glass_sfx_wav_buf &&
                        game->glass_sfx_wav_len <= (Uint32)SDL_MAX_SINT32) {
                        SDL_ClearAudioStream(game->glass_sfx_audio_stream);
                        SDL_PutAudioStreamData(game->glass_sfx_audio_stream,
                                               game->glass_sfx_wav_buf,
                                               (int)game->glass_sfx_wav_len);
                        SDL_ResumeAudioStreamDevice(game->glass_sfx_audio_stream);
                    }
