...
#else /* HAVE_FFMPEG not defined — provide no-op stubs */

VideoPlayer *video_player_open(SDL_Renderer *renderer, const char *path)
{
    (void)renderer; (void)path;
    SDL_Log("video_player_open: FFmpeg not compiled in — jumpscare skipped");
    return NULL;
}

void video_player_update(VideoPlayer *vp, float dt)   { (void)vp; (void)dt; }
void video_player_render(VideoPlayer *vp, SDL_Renderer *r) { (void)vp; (void)r; }
int  video_player_is_done(const VideoPlayer *vp)      { (void)vp; return 1; }
void video_player_close(VideoPlayer *vp)               { (void)vp; }
